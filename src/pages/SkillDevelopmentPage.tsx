...
const govtSchemes = [
  { title: "PM Vishwakarma", description: "The PM Vishwakarma scheme is a Central Sector Scheme to support artisans and craftspeople by providing skill training, access to collateral-free credit, modern tools, and market linkages.", image: pm1 },
  { title: "Swatantrya Amrutha Mahotsava Munnade Yojane", description: '"Amrutha Munnade" is a Karnataka government skill development program aimed at increasing the employability of youth from backward classes by providing free, short-term, skill-based training courses.', image: gs2 },
  { title: "Directorate of Minorities", description: "The Directorate of Minorities in Karnataka focuses on a wide range of projects, including educational support, skill development and training programs, and infrastructure development for minority communities.", image: gs3 },
];
      ...
      <section className="py-20 bg-background">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {govtSchemes.map((s) => (
              <div key={s.title} className="bg-white/10 border border-white/10 backdrop-blur-xl rounded-3xl shadow-xl overflow-hidden transform-gpu transition duration-500 ease-out hover:-translate-y-2 hover:shadow-2xl border-l-4 border-l-accent">
                <img src={s.image} alt={s.title} className="w-full h-44 object-cover" loading="lazy" />
                <div className="p-6">
                  <h3 className="font-bold text-black mb-3">{s.title}</h3>
                  <p className="text-black font-medium text-sm leading-relaxed">{s.description}</p>
                </div>
              </div>
            ))}
          </div>
